...
For this project, we will be looking at diabetes data that includes a lot of risk factors and health-related variables that we can assess for their influence on whether or not a person has diabetes. 
...
```{r}
my_table <- table(factor_data$Diabetes_factor, factor_data$GenHlth_factor)
plot(my_table)
```
...
```{r}
# plot 1
ggplot(data = long_numeric_data,aes(x = Diabetes_factor, y = value, fill = factor_data$Sex_factor))+ #change the second diabetes factor to sex factor if i want to add another variable for sex
  geom_boxplot() +  
  #geom_jitter(width = 0.2, alpha = 0.3, size = 0.01) +
  facet_wrap(~ name,scales = "free_y")
